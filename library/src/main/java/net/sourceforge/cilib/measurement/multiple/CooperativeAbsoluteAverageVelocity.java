 ...
 */
public class CooperativeAbsoluteAverageVelocity implements Measurement {
     ...
     */
    @Override
    public CooperativeAbsoluteAverageVelocity getClone() {
        return this;
    }
            ...
            for (Entity e : currentAlgorithm.getTopology()) {
                ...
                for(int i = 0; i < dimension; i++) {
                    if(averageVelocity.size() < dimension) {
                        averageVelocity.add(velocity.get(i));
                    } else {
                    averageVelocity.set(i, Real.valueOf(averageVelocity.doubleValueOf(i) + velocity.doubleValueOf(i)));
                    }
                }
            }
            for(int i = 0; i < dimension; i++) {
                averageVelocity.set(i, Real.valueOf(Math.abs(averageVelocity.doubleValueOf(i)/(double)dimension)));
            }
